// src/app/core/components/rm-time-field.component.ts
import { Component, ElementRef, EventEmitter, Input, Output, ViewChild } from '@angular/core';

/**
 * Shared themed time picker field. Shows a formatted 12-hour value and opens
 * the native time dialog (system clock on Android, AM/PM handled there) via a
 * hidden input. Value format: "HH:mm" (24-hour).
 */
@Component({
  selector: 'rm-time-field',
  standalone: true,
  template: `
    <button type="button" class="fld" (click)="open()">
      ...
      <input #native class="fld-native" type="time" [value]="value"
  ...
  styles: [`
    ...
    .fld-icon { width: 34px; height: 34px; border-radius: 10px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; background: rgba(245,158,11,.12); color: #F59E0B; }
...
export class RmTimeFieldComponent {
  @Input() label = 'Time';
  @Input() value = '';
  ...
  get display(): string {
    ...
    if (isNaN(h) || isNaN(m)) return this.placeholder;
    const ap = h < 12 ? 'AM' : 'PM';
